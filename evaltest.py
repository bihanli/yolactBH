...
def parse_args(argv=None):
    ...
    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=True, output_web_json=True, shuffle=False,
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    output={}
    output["shapes"]=[]
    #output["shapes"]={"label":None,"point":None}
    output["imagePath"]=str(x)+".png"
    output["imageHeight"]=874
    output["imageWidth"]=1164
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        if num_dets_to_consider > 1:
            ...
            #print(np.shape(masks_color_cumul),np.shape(masks_color_summand))
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                ...
                out={"label":None,"point":None}
                out["label"]=str(_class)
                output["shapes"].append(out)
                #output["shapes"]["point"]=str(masks)
                
                #output[str(x)]['dets'].append({    'score': str(score),
                #                'bbox': str(boxes[j,:]),
                #                'category': str(_class),
                #                })
    #with open('/content/drive/MyDrive/data/json/'+str(x)+'.json','w') as f:
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    ...
    with timer.env('Postprocess'):
        ...
        boxes = boxes.cuda()
        mask_=masks.view(-1, h, w).cpu().numpy()
