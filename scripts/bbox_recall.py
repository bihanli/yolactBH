...
def make_priors(conv_size, scales, aspect_ratios):
    ...
    for j, i in product(range(conv_h), range(conv_w)):
        x = (i + 0.5) / conv_w
        y = (j + 0.5) / conv_h
        for ars in aspect_ratios:#1,0.5,2
            for scale in scales:#[24,30,38]*1.*2.*4.*8.*16
...
scales = [[i * 2 ** (j / 3.0) for j in range(3)] for i in [24, 48, 96, 192, 384]]#[[19.76,53.16,92.7],[131.24,168.79,198.79],[238.15,269.53,306.62],[339.94,372.57,407.11],[444.21,497.11,540.09]]#
aspect_ratios = [ [[1.07,0.5,2.35]] ]*5
conv_sizes = [(69, 69), (35, 35), (18, 18), (9, 9), (5, 5)]
#conv_sizes = [(550,550), (550,550), (550,550), (550,550), (550,550)]
max_size=550
SMALL = 0
MEDIUM = 1
LARGE = 2
