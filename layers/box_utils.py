...
def intersect(box_a, box_b):
    ...
    A = box_a.size(1)
    B = box_b.size(1)
    #print(33,box_a.size(),box_b.size())
    #box_a.size()1,,4
    #box_b.size()1,57744,4
    max_xy = torch.min(box_a[:, :, 2:].unsqueeze(2).expand(n, A, B, 2),
                       box_b[:, :, 2:].unsqueeze(1).expand(n, A, B, 2))#1,,57744,2
...
def jaccard(box_a, box_b, iscrowd:bool=False):
    ...
    if box_a.dim() == 2:#T
        ...
        box_a = box_a[None, ...]
        box_b = box_b[None, ...]
    #print(22,box_a.size(),box_b.size())
    ...
    #print(out.squeeze(0).size())1,57744
    return out if use_batch else out.squeeze(0)#F
...
def match(pos_thresh, neg_thresh, truths, priors, labels, crowd_boxes, loc_t, conf_t, idx_t, idx, loc_data):
    ...
    """
    decoded_priors = decode(loc_data, priors, cfg.use_yolo_regressors) if cfg.use_prediction_matching else point_form(priors)#F左上右下
    
    # Size [num_objects, num_priors]
    overlaps = jaccard(truths, decoded_priors) if not cfg.use_change_matching else change(truths, decoded_priors)#T(FF)

    # Size [num_priors] best ground truth for each prior
    best_truth_overlap, best_truth_idx = overlaps.max(0)#每列max
