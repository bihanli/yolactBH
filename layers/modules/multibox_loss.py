...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, num_classes, pos_threshold, neg_threshold, negpos_ratio):
        ...
        self.num_classes = num_classes#class14
        ...
        if cfg.use_class_balanced_conf:#F
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        if cfg.mask_type == mask_type.lincomb:#1,1
            proto_data = predictions['proto']

        score_data = predictions['score'] if cfg.use_mask_scoring   else None#F   
        inst_data  = predictions['inst']  if cfg.use_instance_coeff else None#F
        
        labels = [None] * len(targets) # Used in sem segm loss len(targets)8
        
        batch_size = loc_data.size(0)#batch size8
        num_priors = priors.size(0)#57744
        num_classes = self.num_classes

        # Match priors (default boxes) and ground truth boxes
        # These tensors will be created with the same device as loc_data
        loc_t = loc_data.new(batch_size, num_priors, 4)#create相同數據類型的tensor
        ...
        for idx in range(batch_size):
            ...
            labels[idx] = targets[idx][:, -1].data.long()#tensor投射為long類型
        ...
        if cfg.train_masks:#T
            if cfg.mask_type == mask_type.direct:#F1,0
                ...
                if cfg.use_maskiou:#T
                    ...
                    loss = ret
                losses.update(loss)#BM
                
                if cfg.mask_proto_loss is not None:#F
        ...
        # Mask IoU Loss
        if cfg.use_maskiou and maskiou_targets is not None:#T
    ...
    def semantic_segmentation_loss(self, segment_data, mask_t, class_t, interpolation_mode='bilinear'):
        ...
        for idx in range(batch_size):
            ...
            j=cur_segment.unsqueeze(1)
            j = torchvision.utils.make_grid(j)
            j=feature_imshow(j)
            #j=j.permute(1,2,0).cpu()
            j=j[:,:,0]
            #j=j.cpu().detach().numpy()
                        
            j=Image.fromarray((j*255).astype(np.uint16))
            #j=cv2.cvtColor(j,cv2.COLOR_BGR2GRAY)
            plt.imshow(j)
            plt.axis('off')
            plt.savefig("cur_segment.png")
            plt.show()
            
            print(segment_t.shape)
            j=segment_t.unsqueeze(1)
            j = torchvision.utils.make_grid(j)
            j=feature_imshow(j)
            #j=j.permute(1,2,0).cpu()
            j=j[:,:,0]
    ...
    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        ...
        if cfg.ohem_use_most_confident:#F
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, interpolation_mode='bilinear'):
        mask_h = proto_data.size(1)#bs,138,138,32
        mask_w = proto_data.size(2)

        process_gt_bboxes = cfg.mask_proto_normalize_emulate_roi_pooling or cfg.mask_proto_crop#T&T
        #T
        if cfg.mask_proto_remove_empty_masks:#F
        ...
        for idx in range(mask_data.size(0)):#bs,57744,32
            with torch.no_grad():
                downsampled_masks = F.interpolate(masks[idx].unsqueeze(0), (mask_h, mask_w),
                                                  mode=interpolation_mode, align_corners=False).squeeze(0)#masks,550,550
                downsampled_masks = downsampled_masks.permute(1, 2, 0).contiguous()

                if cfg.mask_proto_binarize_downsampled_gt:#T
                    downsampled_masks = downsampled_masks.gt(0.5).float()
                ...
                if cfg.mask_proto_reweight_mask_loss:#F
            ...
            if cfg.mask_proto_coeff_diversity_loss:#F
                if inst_data is not None:#Fnone
            ...
            old_num_pos = proto_coef.size(0)#,32
            
            if old_num_pos > cfg.masks_to_train:#F,100
                perm = torch.randperm(proto_coef.size(0))
                select = perm[:cfg.masks_to_train]

                proto_coef = proto_coef[select, :]#100,32
                pos_idx_t  = pos_idx_t[select]#100
            ...
            j=torch.clamp(pred_masks, 0, 1).permute(2,0,1).cpu()
            j=j.unsqueeze(1)
            j = torchvision.utils.make_grid(j)
            j=j.permute(1,2,0).cpu()
            j=j[:,:,0]
            ...
            plt.imshow(j)
            plt.axis('off')
            plt.savefig("pred.png")#sssss
            plt.show()
            #===============================
            j=mask_t.permute(2,0,1).cpu()
            j=j.unsqueeze(1)
            j = torchvision.utils.make_grid(j)
            j=j.permute(1,2,0).cpu()
            j=j[:,:,0]
            ...
            exit()
            """
            if cfg.mask_proto_mask_activation == activation_func.sigmoid:#T
            ...
            if cfg.mask_proto_normalize_mask_loss_by_sqrt_area:#F
            ...
            if cfg.mask_proto_normalize_emulate_roi_pooling:#F
            ...
            if cfg.use_maskiou:#T
                if cfg.discard_mask_area > 0:#T5*5
        ...
        if cfg.use_maskiou:#T
            ...
            if cfg.maskious_to_train > 0 and num_samples > cfg.maskious_to_train:#F
