...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='weights/ssd300_mAR_77.43_v2.pth', type=str,
    ...
    parser.add_argument('--ar_data_file', default='results/ar_data.pkl', type=str,
                        help='In quantitative mode, the file to save detections before calculating mAR.')
    parser.add_argument('--resume', dest='resume', action='store_true',
                        help='If display not set, this resumes mAR calculations from the ar_data_file.')
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_text or args.display_bboxes:
     ...
     #           output[str(x)]['dets'].append({    'score': str(score),
    #                            'bbox': str(boxes[j,:]),
    #                            'category': str(_class),
    #                            })
...
def prep_coco_cats():
    ...
    for coco_cat_id, transformed_cat_id_p1 in get_label_map().items():
        ...
        print(coco_cat_id,transformed_cat_id_p1,transformed_cat_id)
    print(coco_cats)
...
def prep_metrics(ar_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    """ Returns a list of ARs for this image, with each element being for a class  """
    ...
    for _class in set(classes + gt_classes):
        ...
        for iouIdx in range(len(iou_thresholds)):
            ...
            for iou_type, iou_func, crowd_func, score_func, indices in iou_types:
                ...
                ar_obj = ar_data[iou_type][iouIdx][_class]
                ar_obj.add_gt_positives(num_gt_for_class)
                ...
                for i in indices:
                    ...
                    if max_match_idx >= 0:
                        ...
                        if not matched_crowd:
                            ar_obj.push(score_func(i), False)
    timer.stop('Main loop')


class ARDataObject:
    """
    Stores all the information necessary to calculate the AR for one IoU and one class.
    ...
    def get_ar(self) -> float:
        ...
        #print("123",len(precisions))
        #for i in precisions:
        #print("456",sum([1 for i in precisions]))
        ...
        indices = np.searchsorted(precisions, x_range, side='left')
        for bar_idx, recalls_idx in enumerate(indices):
            if recalls_idx < len(recalls):
                y_range[bar_idx] = recalls[recalls_idx]
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if not args.display and not args.benchmark:
        ...
        # Index ar_data[type][iouIdx][classIdx]
        ar_data = {
            'box' : [[ARDataObject() for _ in cfg.dataset.class_names] for _ in iou_thresholds],
            'mask': [[ARDataObject() for _ in cfg.dataset.class_names] for _ in iou_thresholds]
        }
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                ...
                prep_metrics(ar_data, preds, img, gt, gt_masks, h, w, num_crowd, dataset.ids[image_idx], detections)
        ...
        #print(args.display,args.benchmark,args.output_coco_json)
        if not args.display and not args.benchmark:
            print()
            if args.output_coco_json:
                ...
                if not train_mode:
                    print('Saving data...')
                    with open(args.ar_data_file, 'wb') as f:
                        pickle.dump(ar_data, f)

                return calc_mar(ar_data)
        elif args.benchmark:
            print()
            print()
...
def calc_mar(ar_data):
    print('Calculating mAR...')
    ars = [{'box': [], 'mask': []} for _ in iou_thresholds]
    ...
    # Uncomment this for mAR of person for model trained on whole 80 classes.
    # for _class in range(1):
        for iou_idx in range(len(iou_thresholds)):
            for iou_type in ('box', 'mask'):
                ar_obj = ar_data[iou_type][iou_idx][_class]

                if not ar_obj.is_empty():
                    ars[iou_idx][iou_type].append(ar_obj.get_ar())

    all_mars = {'box': OrderedDict(), 'mask': OrderedDict()}
    a=np.zeros((2,10))
    # Looking back at it, this code is really hard to read :/
    for j,iou_type in enumerate(['box', 'mask']):
        all_mars[iou_type]['all'] = 0 # Make this first in the ordereddict
        for i, threshold in enumerate(iou_thresholds):
            mAR = sum(ars[i][iou_type]) / len(ars[i][iou_type]) * 100 if len(ars[i][iou_type]) > 0 else 0
            all_mars[iou_type][int(threshold*100)] = mAR
            a[j][i]=mAR
        all_mars[iou_type]['all'] = (sum(all_mars[iou_type].values()) / (len(all_mars[iou_type].values())-1))
        import csv
        r=np.array(a[j])
        r=np.resize(r,(10,1))
        plt.title(iou_type+'mAR')
        plt.xlabel('Precision')
        plt.ylabel('Recall')
        plt.plot(iou_thresholds ,r/100)
        plt.savefig(iou_type+'.png')
        plt.show()    
        plt.clf()
        a=[]
        a.append(all_mars[iou_type]['all'])
        r=np.resize(r,(1,10))
        a.extend(r)
        a=np.array(a)
        with open('mAR.csv','a')as f:
             w=csv.writer(f)
             w.writerow(a)
             f.write("\n")
    ...
    all_mars = {k: {j: round(u, 2) for j, u in v.items()} for k, v in all_mars.items()}
    return all_mars

def print_mars(all_mars):
    # Warning: hacky 
    make_row = lambda vals: (' %5s |' * len(vals)) % tuple(vals)
    make_sep = lambda n:  ('-------+' * n)

    print()
    print(make_row([''] + [('.%d ' % x if isinstance(x, int) else x + ' ') for x in all_mars['box'].keys()]))
    print(make_sep(len(all_mars['box']) + 1))
    for iou_type in ('box', 'mask'):
        print(make_row([iou_type] + ['%.2f' % x if x < 100 else '%.1f' % x for x in all_mars[iou_type].values()]))
    print(make_sep(len(all_mars['box']) + 1))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if args.resume and not args.display:
            with open(args.ar_data_file, 'rb') as f:
