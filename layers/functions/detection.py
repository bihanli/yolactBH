...
class Detect(object):
    ...
    def __init__(self, num_classes, bkg_label, top_k, conf_thresh, nms_thresh):
        ...
        self.use_cross_class_nms = False#True #
        self.use_fast_nms = True #False#
    ...
    def __call__(self, predictions, net):
        ...
        with timer.env('Detect'):
            ...
            for batch_idx in range(batch_size):
                ...
                result = self.detect(batch_idx, conf_preds, decoded_boxes, mask_data, proto_data, inst_data)
        ...
        #for i in out:
        #  print(i['detection']['mask'].shape,i['detection']['proto'].shape,i['detection']['box'].shape)
        return out


    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, proto_data, inst_data):
        """ Perform nms for only the max scoring class that isn't background (class 0) """
        cur_scores = conf_preds[batch_idx, 1:, :]
        conf_scores, _ = torch.max(cur_scores, dim=0)
        #print(batch_idx, conf_scores.shape, decoded_boxes.shape, mask_data.shape,cur_scores.shape)
        keep = (conf_scores > self.conf_thresh)
        #print(keep,cur_scores.shape)
        scores = cur_scores[:, keep]
        boxes = decoded_boxes[keep, :]
        masks = mask_data[batch_idx, keep, :]
        
        #print(scores.shape,boxes.shape,masks.shape)#,proto.shape)
        if inst_data is not None:
            inst = inst_data[batch_idx, keep, :]
    
        if scores.size(1) == 0:
            return None
        #print(self.use_fast_nms,self.use_cross_class_nms)
        if self.use_fast_nms:
            if self.use_cross_class_nms:
                boxes, masks, classes, scores = self.cc_fast_nms(boxes, masks, scores) 
            else:
                boxes, masks, classes, scores = self.fast_nms(boxes, masks, scores, self.nms_thresh, self.top_k)
                #print(boxes.shape, masks.shape, classes.shape, scores.shape)
        else:
        ...
        #return {'box': boxes, 'mask': masks, 'score': scores}
    ...
    def cc_fast_nms(self, boxes, masks, scores, iou_threshold:float=0.5, top_k:int=200):
        ...
        boxes_idx = boxes[idx]
        #print(11,boxes_idx.size())
        # Compute the pairwise IoU between the boxes
        iou = jaccard(boxes_idx, boxes_idx) #box iou
        
        # Zero out the lower triangle of the cosine similarity matrix and diagonal
        iou.triu_(diagonal=1) #iou upper triangle
